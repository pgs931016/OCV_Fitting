...
Ddvdq77 = diff(dvdq77_moving_avg)./diff(x_values);
Ddvdq88 = diff(dvdq88_moving_avg)./diff(x_values);
figure(10)
plot(x_values(1:end-1), Ddvdq77, 'b-'); hold on;
plot(x_values(1:end-1), Ddvdq88, 'r-');
...
% 두 번째 x 범위
x_min2 = 0.25; % 다른 최소 x 범위
x_max2 = 0.3; % 다른 최대 x 범위
...
%네 번째 x 범위
x_min4 = 0.75; % 다른 최소 x 범위
x_max4 = 0.85; % 다른 최대 x 범위
...
% 별 마커와 텍스트 추가
x_star =[corresponding_x1,corresponding_x2,corresponding_x3,corresponding_x4]; 
...
% 네모 마커와 텍스트 추가
x_square = [corresponding_x11,corresponding_x22,corresponding_x33,corresponding_x44]; 
...
maxError = max([results(soc_idx).e5,results(soc_idx).e6,results(soc_idx).e7,results(soc_idx).e8]);
...
% 별 마커와 텍스트 추가
x_star = [results(soc_idx).corresponding_x1,results(soc_idx). corresponding_x2,results(soc_idx).corresponding_x3,results(soc_idx). corresponding_x4]; 
roundedValue1 = round(x_star * 10^3) / 10^3;
y_star = results(soc_idx).dvdq77_moving_avg([results(soc_idx). index1,results(soc_idx). index2, results(soc_idx). index3, results(soc_idx). index4]); 
star_handle = plot(x_star, y_star, 'p', 'MarkerEdgeColor', 'k', 'MarkerFaceColor', 'b', 'MarkerSize', 12, 'DisplayName', 'Data Peak');
text(x_star, zeros(size(x_star))+0.2, arrayfun(@num2str, roundedValue1, 'UniformOutput', false), 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'center', 'Color', 'b','FontSize', 8); 


% 네모 마커와 텍스트 추가
x_square = [results(soc_idx). corresponding_x11,results(soc_idx).corresponding_x22,results(soc_idx). corresponding_x33,results(soc_idx). corresponding_x44]; 
roundedValue2 = round(x_square * 10^3) / 10^3; 
y_square = results(soc_idx).dvdq88_moving_avg([results(soc_idx). index11,results(soc_idx). index22,results(soc_idx). index33,results(soc_idx). index44]); 
square_handle = plot(x_square, y_square, 's', 'MarkerEdgeColor', 'k', 'MarkerFaceColor', 'r', 'MarkerSize', 8, 'Color', [1, 0, 0, 0.5], 'DisplayName', 'Fit Peak');
text(x_square, zeros(size(x_square))+0.1, arrayfun(@num2str, roundedValue2, 'UniformOutput', false), 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'center','Color', 'r','FontSize', 8);  
